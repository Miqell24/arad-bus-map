...
function wayAccess(tags) {
  ...
  if (tags.highway === 'service' && !psv) {
    const s = tags.service;
    if (s === 'drive-through') return null;
    // Parking aisles ride along with the driveway penalty instead of being cut:
    // Arad's 48 serves Selgros and Real INSIDE their car parks and the Ghioroc
    // bus calls at La Cocoș on the same lot — with the aisles missing the
    // route broke into stubs there (user report). The ×4 cost keeps the
    // aisles out of every route whose shape does not actually run on them.
    driveway = s === 'driveway' || s === 'parking_aisle';
  }
