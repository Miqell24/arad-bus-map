...
// Poprawki nazw słupków, gdyby feed nazwał któryś źle. Arad pisze po
// rumuńsku z diakrytykami i nic nie wymaga transliteracji — pusto.
const NAME_FIX = {};
...
// Numer linii jest w Aradzie dosłownie tym, co na tabliczce, więc słownik
// etykiet stoi pusty, a żadna linia nie jest „pniem szynowym" (to tryb dla
// metra i kolei miejskich — tramwaje rysują się normalnie).
const LBL = new Map();
const isRailTrunk = () => false;

// DWA tryby, JEDEN feed.
//
// Arad nie wydaje GTFS-a. Feed pod data/gtfs pisze pipeline/arad-feed.mjs
// z arkuszy zamawiającego (Anexa 17 — słupki, TRASEE URBANE — trasy),
// z rozkładów operatora (ctparad.ro, eksport BusMana) i z plannera
// pasażerskiego CTP na platformie Telelink City (współrzędne brakujących
// słupków i polilinie wzorców).
//
// Tramwaje jadą po szynach (railway=tram), autobusy po jezdniach, więc każdy
// tryb dostaje własny graf i własny wycinek OSM.
//
// Cztery linie z numerem tramwajowym — 7A, 18A, Vladimirescu i Ghioroc —
// operator prowadzi AUTOBUSAMI (na stronie rozkładów mają przedrostek
// „AUTOBUZ:"), i tak też są w feedzie: route_type 3, graf drogowy, granat
// rodziny. Ghioroc i Vladimirescu to dawna linia tramwaju podmiejskiego
// Arad–Ghioroc; ich przystanki nadal stoją przy torze wzdłuż DN7.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/arad.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['19']),
  feeds: [
    { tag: 'ctp', dir: 'data/gtfs', routeTypes: ['3'], trustDir: true, mapKey: (sn) => (sn || '').trim() || null },
  ],
}];
...
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/arad-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'ctp', dir: 'data/gtfs', routeTypes: ['0'], trustDir: true, mapKey: (sn) => (sn || '').trim() || null },
  ],
});
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    for await (const t of iterCsv(join(fdir, 'trips.txt'))) {
      ...
      // feed.trustDir: kierunki pisze arad-feed.mjs z bloków rozkładu BusMana,
      // więc direction_id jest tu wiarygodne także tam, gdzie linia jest pętlą
      // i ma tylko jedną wartość — bez tego linie 7, 18b, 48 i 52 rozpadłyby się
      // na tyle „kierunków", ile różnych końcówek mają kursy skrócone
      const dir = (feed.trustDir || (dirSeen.get(L)?.size ?? 0) > 1) ? (t.direction_id || '0') : hsKey(t);
      let m = dirs.get(dir);
    ...
    // (the SHAPE_GAP_MAX decision itself is taken in the matching step, AFTER
    // the on-road legs of an overview polyline have been densified — see
    // densifyOnRoad below; here only a shape with nothing in it falls back)
    const worstGap = (pts) => {
      let w = 0;
  ...
  // An OVERVIEW polyline (the planner's, here Telelink's — simplified the way a
  // routing API returns one) keeps its vertices at the bends and drops them
  // along straight roads, so a leg of 300 m to 2 km is usually a straight road
  // with no vertex on it. Bridging such a leg by routing draws the SHORTEST
  // path between its ends, and on a street grid that is sometimes the parallel
  // street, not the bus's (49b along Economu, user report); past SHAPE_GAP_MAX
  // the whole shape used to be thrown away (the Ghioroc run, one 2.3 km leg
  // on the dead-straight DN7). So a long leg whose chord lies ON the road —
  // every 20 m point within ROAD_NEAR of a graph segment — is densified into
  // ordinary observations and followed like the rest of the shape; a chord
  // across the fields (Burgas's 3.7 km to Banevo) stays a gap for the router,
  // and only what is still a gap after that counts against SHAPE_GAP_MAX.
  const ROAD_NEAR = 30, SHAPE_GAP_MAX = 2000;
  const nearSeg = (() => {
    const cell = 150;
    const grid = new Map();
    graph.segs.forEach((sg, i) => {
      const x0 = Math.floor(Math.min(sg.ax, sg.bx) / cell), x1 = Math.floor(Math.max(sg.ax, sg.bx) / cell);
      const y0 = Math.floor(Math.min(sg.ay, sg.by) / cell), y1 = Math.floor(Math.max(sg.ay, sg.by) / cell);
    ...
    const segD = (px, py, sg) => {
      ...
      let t = l2 ? ((px - sg.ax) * dx + (py - sg.ay) * dy) / l2 : 0;
      t = Math.max(0, Math.min(1, t));
      return Math.hypot(px - (sg.ax + t * dx), py - (sg.ay + t * dy));
    };
    return (px, py) => {
      let best = Infinity;
      const cx = Math.floor(px / cell), cy = Math.floor(py / cell);
      for (let x = cx - 1; x <= cx + 1; x++) for (let y = cy - 1; y <= cy + 1; y++) {
        for (const i of grid.get(x + ',' + y) ?? []) { const d = segD(px, py, graph.segs[i]); if (d < best) best = d; }
      }
      return best;
    };
  })();
  // The chord is only TESTED here, never turned into observations: a chord
  // that runs 30 m from some road is not necessarily 30 m from the bus's road
  // — between Podgoria and Uzina Electrică the Ghioroc chord lies along the
  // residential Iuliu Toader while the bus keeps to the curving Calea Radnei,
  // and densified chord points dragged the line through the side streets
  // (user report). Long legs are bridged by routing, as everywhere in the
  // family; the on-road test only decides whether a leg is a straight road
  // (bridge it) or a chord across open country (counts against SHAPE_GAP_MAX).
  const densifyOnRoad = (xy) => {
    let legs = 0, meters = 0, left = 0, worst = 0;
    for (let i = 1; i < xy.length; i++) {
      const [ax, ay] = xy[i - 1], [bx, by] = xy[i];
      const L = Math.hypot(bx - ax, by - ay);
      if (L <= GAP_MIN) continue;
      const n = Math.ceil(L / 20);
      let onRoad = true;
      for (let k = 1; k < n; k++) {
        const t = k / n;
        if (nearSeg(ax + t * (bx - ax), ay + t * (by - ay)) > ROAD_NEAR) { onRoad = false; break; }
      }
      if (onRoad) { legs++; meters += L; } else { left++; if (L > worst) worst = L; }
    }
    return { out: xy, legs, meters, left, worst };
  };

  for (const r of reps) {
    let xy = r.shapeLatLon.map(([lat, lon]) => proj.toXY(lat, lon));
    if (!r.pseudo) {
      const d = densifyOnRoad(xy);
      if (d.legs || d.left) log(`  shape legs ${r.line}/${r.dir}: ${d.legs} long leg(s) on a straight road (${Math.round(d.meters)} m), bridged by routing` +
        (d.left ? `; ${d.left} chord(s) across open country (longest ${Math.round(d.worst)} m)` : ''));
      xy = d.out;
      if (d.worst > SHAPE_GAP_MAX) {
        log(`${r.line}/${r.dir}: overview polyline still jumps ${Math.round(d.worst)} m across open country — matching on the stop sequence instead`);
        r.pseudo = true;
        r.shapeOrig = r.shapeLatLon;
        r.shapeLatLon = r.stopSeq.map((s) => stopsById.get(s.stopId)).filter(Boolean).map((st) => [st.lat, st.lon]);
        xy = r.shapeLatLon.map(([lat, lon]) => proj.toXY(lat, lon));
      }
    }
    let sampled, opts;
    if (r.pseudo) {
      ...
      // (a softer emission for the trams was tried against the planner's
      // road-drawn polylines and rejected: on double track it let the path
      // flip between the two rails; the junction curves are handled by the
      // matcher's switchback pass instead)
    }
    const stopsXY = r.stopSeq
      .map((s) => stopsById.get(s.stopId))
      .filter(Boolean)
      .map((st) => proj.toXY(st.lat, st.lon));
    // the matcher's hairpin pass spares a reversal that is a real turnaround —
    // a cul-de-sac with one of THIS run's stops at its end
    opts.stopsXY = stopsXY;
    ...
    log(`line ${r.line} dir ${r.dir}: ${r.lengthKm.toFixed(2)} km, mean error ${res.stats.meanError.toFixed(1)} m, ` +
        ...
        (res.stats.hairpins ? `hairpins=${res.stats.hairpins} fixed, ` : '') + (res.stats.hairpinsLeft ? `hairpins LEFT=${res.stats.hairpinsLeft}, ` : '') +
  ...
  for (const r of reps) {
    r.stopSeq.forEach((s, i) => {
      ...
      if (!e) stopAgg.set(s.stopId, (e = { id: s.stopId, name: st.name, lat: st.lat, lon: st.lon, lines: new Set(), runs: new Set(), term: new Set() }));
      e.lines.add(r.line);
      e.runs.add(r);
  ...
  for (const e of stopAgg.values()) {
    ...
    stopFeatures.push({
      ...
      properties: {
        name: e.name,
        // GTFS stop_id (feed tag stripped) — the departure board in the popup
        // is keyed by it (data/out/timetables.json, pipeline/timetables.mjs)
        ...(e.id ? { sid: String(e.id).replace(/^[^:]*:/, '') } : {}),
        lines: arr.join(', '),
        arr,
        terminus,
  ...
  // the QA layer shows what the FEED says the route is: a rep matched on its
  // stop sequence has no such thing (line 13's two poles are not a shape), and
  // one that fell back keeps its original polyline on the layer
  // one matched path per line and direction, for pipeline/shape-check.mjs —
  // route.geojson merges runs across lines, so a per-line QA needs this
  const pathFeatures = reps.filter((r) => r.matchedXY?.length > 1).map((r) => ({
    type: 'Feature',
    geometry: { type: 'LineString', coordinates: r.matchedXY.map(([x, y]) => { const [lon, lat] = proj.toLonLat(x, y); return [round6(lon), round6(lat)]; }) },
    properties: { line: r.line, dir: r.dir, mode: cfg.mode, pseudo: r.pseudo ? 1 : 0 },
  }));
  const shapeFeatures = reps.filter((r) => r.shapeOrig || !r.pseudo).map((r) => ({
    type: 'Feature',
    geometry: { type: 'LineString', coordinates: (r.shapeOrig ?? r.shapeLatLon).map(([lat, lon]) => [lon, lat]) },
  ...
  return { routeFeatures, shapeFeatures, pathFeatures, stopFeatures, streetFeatures, badgeAnchors, metaLines };
}
...
const pathFeatures = results.flatMap((r) => r.pathFeatures);
...
const BADGE_BANDS = [[13, 13.6], [13.6, 14.4], [14.4, 15.5], [15.5, 16.8], [16.8, 22]];
{
  ...
  BADGE_BANDS.forEach(([z0], band) => {
    ...
    for (const c of cl) {
      ...
      // Cells are as wide as their label needs. Arad's two nameless services
      // print „Ghioroc" and „Vladimirescu", and in a fixed 3.4 em cell the two
      // boxes sat on top of each other at the city's main terminus; short
      // numbers keep the old spacing, so nothing else moves.
      const cellW = (txt) => Math.max(CELL_W, 1.2 + 0.62 * String(txt).length);
      lines.forEach((l, i) => {
        const row = Math.floor(i / pRow), col = i % pRow;
        const rowItems = lines.slice(row * pRow, row * pRow + pRow);
        const widths = rowItems.map((x) => cellW(LBL.get(x.line) ?? x.line));
        const total = widths.reduce((a, b) => a + b, 0);
        let cx = -total / 2 + widths[col] / 2;
        for (let k = 0; k < col; k++) cx += widths[k];
...
writeFileSync(join(outDir, 'paths.geojson'), fc(pathFeatures));
