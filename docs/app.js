...
// font — Roboto Condensed is the hosted narrow equivalent (Romanian diacritics included).
...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: CTP Arad (ctparad.ro) · stops: Anexa 17' }));
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · Timetables: CTP Arad (ctparad.ro) · stops: Anexa 17';
      ...
      a.download = `arad-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `arad-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // Departure boards. data/timetables.json (pipeline/timetables.mjs) carries,
  // per pole, line and direction, the minutes the operator publishes — the
  // feed's stop_times are CTP's own timetables, so the popup can show what
  // hangs on the pole in the street: hour rows, one column per day type.
  // Loaded once, on the first click.
  let ttData = null, ttLoading = null;
  const loadTT = () => (ttLoading ??= fetch('data/timetables.json')
    .then((r) => (r.ok ? r.json() : {})).catch(() => ({})).then((j) => (ttData = j)));
  const TT_DAYS = [['L', 'Mon–Fri'], ['S', 'Sat'], ['D', 'Sun']];
  const boardHtml = (entries, mode) => {
    // several poles can share one point (Anexa 17 gives Cicir 1/2 or
    // Mândruloc 1/2 identical coordinates): their boards are shown together
    const boards = entries.flatMap((e) => (e && e.boards) || []);
    if (!boards.length) return '<p class="tt-none">No published departures from this pole.</p>';
    return '<div class="tt-wrap">' + boards.map((b, i) => {
      const hours = [...new Set(TT_DAYS.flatMap(([k]) => Object.keys(b[k] || {}).map(Number)))].sort((x, y) => x - y);
      const cell = (k, h) => (b[k] && b[k][h] ? b[k][h].map((m) => String(m).padStart(2, '0')).join(' ') : '<span class="tt-dash">–</span>');
      const rows = hours.map((h) => `<tr><th>${h % 24}</th>${TT_DAYS.map(([k]) => `<td>${cell(k, h)}</td>`).join('')}</tr>`).join('');
      return `<details class="tt"${i === 0 ? ' open' : ''}><summary><span class="tt-line${mode === 'tram' ? ' tram' : ''}">${esc(disp(b.line))}</span> → ${esc(b.to)}</summary>` +
        `<table><thead><tr><th></th>${TT_DAYS.map(([, n]) => `<th>${n}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table></details>`;
    }).join('') + '</div>';
  };
  map.on('click', 'stops-dots', (e) => {
    const f = e.features[0];
    const p = f.properties;
    // every pole drawn under the click (co-located poles of both directions)
    const here = map.queryRenderedFeatures(e.point, { layers: ['stops-dots'] })
      .filter((g) => g.properties.name === p.name);
    const sids = [...new Set(here.map((g) => g.properties.sid).filter(Boolean))];
    const lines = [...new Set(here.flatMap((g) => String(g.properties.lines || '').split(', ')).filter(Boolean))];
    const label = lines.length > 1 ? 'lines' : 'line';
    const head = `<strong>${esc(p.name)}</strong>${p.terminus ? ' · terminus' : ''}<br>${label}: ${esc(lines.join(', '))}`;
    const popup = new maplibregl.Popup({ closeButton: true, offset: 10, maxWidth: '380px' })
      .setLngLat(f.geometry.coordinates)
      .setHTML(head + '<p class="tt-none">Loading departures…</p>')
      .addTo(map);
    loadTT().then(() => {
      if (!popup.isOpen()) return;
      popup.setHTML(head + boardHtml(sids.map((id) => ttData[id]), p.mode));
    });
  });
  ...
  map.jumpTo({ center: [21.3150, 46.1800], zoom: 12.4 });
}
